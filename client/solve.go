...
func (c *Client) solve(ctx context.Context, def *llb.Definition, runGateway runGatewayCB, opt SolveOpt, statusChan chan *SolveStatus) (*SolveResponse, error) {
	...
	if !opt.SessionPreInitialized {
		...
		for exID, ex := range opt.Exports {
			...
			if supportStore {
				...
				// When intermediate-images is enabled with OCI directory output,
				// wrap the store so Update() calls carrying a step descriptor
				// label incrementally update index.json during the build.
				if ex.Type == ExporterOCI && ex.OutputDir != "" {
					if v := opt.FrontendAttrs["intermediate-images"]; v != "" && v != "false" {
						store = &indexUpdatingStore{Store: store, storePath: ex.OutputDir}
					}
				}
	...
	// signal the server to enable Docker streaming by setting the
	// intermediate-images attribute value to "docker-stream". This is only done when
	// no OCI direct output is configured: OCI tars cannot be extended mid-build
	// (batch export is used instead), and OCI directories use the content-store
	// per-step push path (server derives this from the exporter configuration).
	...
	if opt.IntermediateImageOutput != nil {
		...
		if !hasOCIDirectOutput {
			frontendAttrs["intermediate-images"] = "docker-stream"
		}
	}
	...
	// In OCI dir + intermediate-images mode, each step's descriptor was already
	// written to index.json per-step via indexUpdatingStore. Skip the batch
	// response key to avoid duplicates (ociindex.Put does not deduplicate
	// unnamed descriptors).
	ociPushUsed := false
	if v := opt.FrontendAttrs["intermediate-images"]; v != "" && v != "false" {
		for _, ex := range opt.Exports {
